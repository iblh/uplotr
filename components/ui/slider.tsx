...
const Slider = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SliderPrimitive.Root> & { variant?: 'default' | 'playback' }
  ...
  >
    <SliderPrimitive.Track className="relative h-2 w-full grow overflow-hidden rounded-full bg-primary">
      <SliderPrimitive.Range className={cn(
        "absolute h-full",
        variant === 'playback' ? "bg-slate-900 dark:bg-primary" : "bg-primary"
      )} />
    ...
    <SliderPrimitive.Thumb className={cn(
      ...
      variant === 'playback' ? "border-slate-900 dark:border-primary" : "border-primary"
    )} />
  </SliderPrimitive.Root>
))
